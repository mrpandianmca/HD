...
if (!empty($_POST) ) {
    ...
    function getData($sql){
        ...
        $result = oci_parse($connection, $sql) OR DIE ("Can't get Data from DB , check your SQL Query " );
        oci_execute($result);
    //     while ( $aRow = oci_fetch_array($result, OCI_ASSOC) )
    //     { 
    //     //$row = array(); 
    //      $data = array();
    ...
    // }
        while (oci_fetch($result)) {
            oci_result($result, 'Ticket_No');
            oci_result($result, 'Name');
            oci_result($result, 'Designation');
            oci_result($result, 'Department_Name');
            oci_result($result, 'Mobile_No');
        }
     

        // foreach ($query as $value) {
        //     oci_bind_by_name($query, ':quantity', $Ticket_No);
        //     oci_bind_by_name($query, ':matric_No', $Name);
        //     oci_bind_by_name($query, ':medicine_ID', $Designation);
        //     oci_bind_by_name($query, ':dates', $Department_Name);
        //     oci_bind_by_name($query, ':dates', $Mobile_No);
